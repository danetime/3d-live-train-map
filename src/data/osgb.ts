 ...
 * lat/long, so berth coordinates supplied as OS grid references can be placed on
 * the map. Uses the standard inverse Transverse Mercator projection followed by
 * a 7-parameter Helmert transformation (OSGB36 → WGS84). Accurate to a few
 * metres without OSTN15 — ample for this stylised map.
 *
 * Adapted from the well-known algorithm in the Ordnance Survey "A guide to
...
export function osGridToLatLng(easting: number, northing: number): LatLng {
...
function helmertOsgb36ToWgs84(lat: number, lon: number, a: number, b: number): LatLng {
  const e2 = 1 - (b * b) / (a * a);
  const sinLat = sin(lat);
  const cosLat = cos(lat);
  ...
  // OSGB36 → WGS84 (inverse of the usual WGS84→OSGB36 parameters).
  const tx = 446.448;
  const ty = -125.157;
  const tz = 542.06;
  const s = -20.4894e-6;
